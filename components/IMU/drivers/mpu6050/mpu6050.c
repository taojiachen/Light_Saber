...
#define MPU6050_DLPF_CONFIG         0x1Au
...
// 设置 DLPF 并获取对应的陀螺仪采样率（参考手册）
// dlpf_cfg: 0~6，0=260Hz, 1=184Hz, 2=94Hz, 3=44Hz, 4=21Hz, 5=10Hz, 6=5Hz
esp_err_t mpu6050_set_dlpf(mpu6050_handle_t sensor, uint8_t dlpf_cfg)
{
    esp_err_t ret;
    uint8_t reg;
    ret = mpu6050_read(sensor, MPU6050_DLPF_CONFIG, &reg, 1);
    if (ret != ESP_OK) return ret;

    // 清除 DLPF 位（bit0-2），保留 EXT_SYNC_SET（bit3-5）和低通滤波位置
    reg &= ~0x07;
    reg |= (dlpf_cfg & 0x07);
    return mpu6050_write(sensor, MPU6050_DLPF_CONFIG, &reg, 1);
}

esp_err_t mpu6050_set_sample_rate_divider(mpu6050_handle_t sensor, uint8_t divider)
{
    return mpu6050_write(sensor, MPU6050_SMPLRT_DIV, &divider, 1);
}
