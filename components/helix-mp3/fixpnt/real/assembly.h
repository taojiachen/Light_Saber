...
/**************************************************************************************
 * MULSHIFT32(x, y)
 * 功能：32位有符号数 x * y，返回64位结果的高32位（等效 (x*y) >> 32）
 * 实现：使用 Xtensa 指令 mulsh（multiply and shift high，有符号乘法取高32位）
 * ESP32-S3 的 Xtensa LX7 核心原生支持 mulsh 指令
 **************************************************************************************/
static __inline int MULSHIFT32(int x, int y)
{
    int ret;
    /* mulsh %0, %1, %2: 计算 %1 * %2（有符号32位），结果高32位存入 %0 */
    __asm volatile ("mulsh %0, %1, %2" : "=r" (ret) : "r" (x), "r" (y));
    return ret;
}
 ...
 * FASTABS(x)
 * 功能：无分支实现有符号整数的绝对值
 * 实现：使用 Xtensa 指令 abs（硬件级绝对值，比软件实现更快）
 * ESP32-S3 的 Xtensa LX7 核心原生支持 abs 指令
 **************************************************************************************/
static __inline int FASTABS(int x)
{
    int ret;
    /* abs %0, %1: 计算 %1 的绝对值，结果存入 %0 */
    __asm volatile ("abs %0, %1" : "=r" (ret) : "r" (x));
    return ret;
}

/**************************************************************************************
 * CLZ(x)
 * 功能：统计32位整数的前导零个数（最高位到第一个1之间的零的个数）
 * 实现：使用 GCC 内置函数 __builtin_clz，ESP32-S3 工具链原生支持
 * 注意：x=0 时返回 32（符合原代码逻辑）
 **************************************************************************************/
static __inline int CLZ(int x)
{
    if (!x) {
        return sizeof(int) * 8; /* x=0 时返回32（32位整数） */
    }
    /* __builtin_clz: GCC 内置函数，统计无符号整数的前导零 */
    return __builtin_clz((unsigned int)x);
}

/**************************************************************************************
 * MADD64(sum, x, y)
 * 功能：64位累加运算 sum += (int64_t)x * y
 * 实现：直接使用 64位整数运算，ESP32-S3 编译器会优化为高效的 Xtensa 指令组合
 **************************************************************************************/
...
/**************************************************************************************
 * SHL64(x, n)
 * 功能：64位整数左移 n 位（逻辑左移，高位补0）
 * 实现：直接使用位移运算符，ESP32-S3 编译器支持 64位位移优化
 **************************************************************************************/
static __inline Word64 SHL64(Word64 x, int n)
{
    return x << n;
...
/**************************************************************************************
 * SAR64(x, n)
 * 功能：64位整数算术右移 n 位（符号位扩展，保持负数符号）
 * 实现：直接使用位移运算符，ESP32-S3 编译器支持 64位算术右移优化
 **************************************************************************************/
static __inline Word64 SAR64(Word64 x, int n)
{
    return x >> n;
